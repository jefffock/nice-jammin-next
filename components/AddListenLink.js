...
export default function AddListenLink({
  song,
  date,
  location,
  tags,
  ...
  const handleSubmit = async () => {
    //todo: move this log to an api, handle errors
    if (!loading) {
      setLoading(true);
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>
          Add a Link for {song} from {date}
        </DialogTitle>
        <Alert severity="info" sx={{ m: "1em" }}>
          Preferred sites: Relisten, YouTube. Link can be to audio only or audio
          + video. Thank you!
        </Alert>
        <DialogContent>
          {/* <DialogContentText>
          ...
          {!user && (
            <Alert severity="warning" sx={{ mb: "1em" }}>
              Please log in to add a link - thank you!
            </Alert>
          )}
          <TextField
            autoFocus
          ...
          />
          {success && (
            <Alert severity="success" sx={{ m: "1em" }}>
              Successfully added link. Thank you! The link will be visible when
